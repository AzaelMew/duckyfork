...
async function getPlayer(player, profile) {
    ...
    const hypixelResponse = await axios.get(`https://api.hypixel.net/v2/skyblock/profiles?uuid=${mojangResponse}&key=${process.env.APIKEY}`);
    ...
    onCommand(username, message) {
      getData(username, message).then(returnurl => {
          ...
          if(returnurl.toString().includes("imgur")){
            this.send(`/gc ${username}: ${Encode(returnurl)}`)
            ...
            this.minecraft.broadcastCleanEmbed({ message: `${returnurl}`, color: 0xF04947 })
          }
      })
  }
}
