...
async function imgurID(url) {
    try {
        ...
        const imageUrl = imageDataArray[0].url;
...
async function getCatPic() {
    try {
        const imgurIDValue = await imgurID("https://api.thecatapi.com/v1/images/search");
        if (imgurIDValue) {
            return imgurIDValue;
        } else {
...
class CatCommand extends minecraftCommand {
    constructor(minecraft) {
        super(minecraft)

        this.name = 'cat'
        this.aliases = ['car']
        this.description = 'Gives cat'
    }
