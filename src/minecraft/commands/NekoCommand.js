...
async function imgurID(url) {
    try {
        ...
        const imageUrl = imageDataArray.results[0].url;
...
async function getCatPic() {
    try {
        const imgurIDValue = await imgurID("https://nekos.best/api/v2/neko");
        if (imgurIDValue) {
            return imgurIDValue;
        } else {
        ...
        return "Failed to get neko picture"; // or handle error appropriately
    }
}
...
class NekoCommand extends minecraftCommand {
    constructor(minecraft) {
        super(minecraft)

        this.name = 'neko'
        this.aliases = ['catgirl']
        this.description = 'Gives catgirl'
    }

    async onCommand(username, message) {
        incrementNumberInJSON("MCCatCommandCount")
        let cat = await getCatPic();
        this.send(`/gc ${Encode(cat)}`)
        this.minecraft.broadcastNewImage({ username: username, image: `${cat}`, icon: 'https://www.mc-heads.net/avatar/' + username })
    }
}
